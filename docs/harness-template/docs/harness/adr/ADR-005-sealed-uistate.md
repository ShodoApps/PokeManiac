...
sealed class MyFeatureUiState {
    ...
    data object EmptySearch : MyFeatureUiState()
    data class Data(val users: PersistentList<MyFeatureUiModel>) : MyFeatureUiState()
}
```
...
- Test assertions are clean: `assertEquals(MyFeatureUiState.Loading, awaitItem())`
