...
[PROJECT_NAME] targets both Android and iOS. Adding iOS as a native SwiftUI app meant either:
...
The app was designed with a clean domain/data/presentation separation (see ADR-001), which makes KMP migration possible without rewriting from scratch.
...
|---|---|---|
| A–C | `:shared:domain` — interfaces + entities | ⏳ Pending |
| D | `:shared:api`, `:shared:data`, `:shared:database`, `:shared:tracking` | ⏳ Pending |
| E | `:shared:di` — Koin modules in commonMain | ⏳ Pending |
| F | `:shared:presentation` — ScreenModels + UiState | ⏳ Pending |
| G | iOS SwiftUI app consuming shared ScreenModels via SKIE | ⏳ Pending |
