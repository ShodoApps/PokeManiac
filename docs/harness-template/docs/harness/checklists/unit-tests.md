...
### Test pattern (reference: `MyFeatureScreenModel`)

```kotlin
class MyFeatureScreenModelTest {
    ...
    private lateinit var screenModel: MyFeatureScreenModel

    @BeforeTest
    fun setUp() {
        screenModel = MyFeatureScreenModel(
    ...
    @Test
    fun `myFeature emits Loading then Data when results found`() = testScope.runTest {
        // Given
        userRepository.searchResults = listOf(mockUser)

        screenModel.uiState.test {
            assertEquals(MyFeatureUiState.EmptySearch, awaitItem()) // initial value

            // When
            screenModel.myFeature("query")
    ...
    @Test
    fun `myFeature emits PresentationError when repository throws`() = testScope.runTest {
        // Given
