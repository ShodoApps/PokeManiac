...
PokeManiac needs local persistence for:
- Caching friend lists and activity feeds (offline support)
- Storing user profile data between sessions
...
Room entities live in `shared:database` `commonMain`. DAOs use `Flow<>` for reads and `suspend` for writes. The `PokeManiacDatabase` is built in `app/di/DatabaseModule.kt` using `Room.databaseBuilder(androidApplication(), ...)` — the Android-specific construction stays at the app level.
