...
PokeManiac is a social app for Pokémon card collectors. It has two clients (Android, iOS), multiple data sources (remote API, Room database), and business logic that must be testable independently of frameworks. Early versions mixed network code, database code, and UI code in the same classes, which made testing slow and brittle.
